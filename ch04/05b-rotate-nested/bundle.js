...
var ships = scene.add(new Container());
ships.rotation = 0.1;
ships.pivot = { x: 16, y: 16 };

// Testing nested rotations

var two = scene.add(new Container());
two.pos.x = w / 2;
two.pos.y = h / 2;
two.rotation = 0.1;
[].concat(_toConsumableArray(Array(10))).map(function (_, i) {
  var twoinner = two.add(new Container());
  twoinner.pivot = { x: 16, y: 16 };
  twoinner.rotation = i * (Math.PI / 6);
  var t = twoinner.add(new Sprite(textures.spaceship));
  t.pos.x = i * 12;
  t.pivot.x = 16;
  t.pivot.y = 16;
...
for (var i = 0; i < 10; i++) {
  var ship = ships.add(new Sprite(textures.spaceship));
  ship.pivot = { x: 16, y: 16 };
  ship.pos.x = i * 48;
}

game.run(function (dt, t) {
  var rps = Math.PI * 2 * dt;
  ships.map(function (s, i) {
    s.rotation += i * rps;
