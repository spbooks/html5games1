...
var GameScreen = function (_Container) {
  ...
  function GameScreen(game, controls) {
    ...
    _this.line = _this.add(new Rect(2, game.h));

    _this.ctd = _this.add(new _CrashTestDummy2.default(_this.bounds));
    _this.ctd.pos.y = _this.h / 2;

    _this.timer = _this.add(new Text("time", {
  ...
  _createClass(CrashTestDummy, [{
    ...
    value: function update(dt) {
      ...
      if (this.time >= 2 && !this.prnt) {
        ...
        var expectedPos = 0.5 * ACCELERATION * this.time * this.time;
