...
var GameScreen = function (_Container) {
  ...
  function GameScreen(game, controls) {
    ...
    _this.balls = _this.add(new Container());
    for (var i = 0; i < 40; i++) {
      var b = _this.balls.add(new _CrashTestDummy2.default(_this.bounds));
      b.pos.set(math.rand(32, _this.w - 64), math.rand(32, _this.h - 64));
    }
    return _this;
  }
  ...
  _createClass(GameScreen, [{
    ...
    value: function update(dt, t) {
      ...
      var balls = this.balls.children;
      for (var i = 0; i < balls.length; i++) {
        var a = balls[i];

        for (var j = i + 1; j < balls.length; j++) {
          var b = balls[j];
          var diff = b.pos.clone().subtract(a.pos);
          if (diff.mag() > a.radius + b.radius) {
            continue;
          }
          var mid = a.pos.clone().add(b.pos).divide(2);
          ...
          var power = (a.vel.x - b.vel.x) * normal.x;
          power += (a.vel.y - b.vel.y) * normal.y;

          var displacement = normal.multiply(power);
