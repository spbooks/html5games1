...
var GameScreen = function (_Container) {
  ...
  function GameScreen(game, controls) {
    ...
    var bounds = { x: 0, y: 0, w: _this.w, h: _this.h };

    for (var i = 0; i < 30; i++) {
      _this.add(new _CrashTestDummy2.default(bounds));
    }
    return _this;
  }
...
var CrashTestDummy = function (_TileSprite) {
  ...
  function CrashTestDummy(bounds) {
    ...
    _this.vel = new Vec(math.rand(-300, 300), math.rand(-300, 300));
    _this.pos.set(bounds.w / 2, bounds.h / 2);
    _this.bounds = bounds;
    _this.rotation = math.rand(4) * (Math.PI / 2);
